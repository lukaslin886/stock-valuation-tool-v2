...
def show_market_screener(data_manager):
    ...
    with col1:
        ...
        min_cap = st.slider("最低市值 (億台幣)", 0, 1000, 50, step=10)
        max_pe = st.slider("最高本益比 (PE)", 5.0, 100.0, 20.0, step=0.5)
        
    with col2:
        st.subheader("2. 收益與成長 (Yield)")
        min_yield = st.slider("最低殖利率 (%)", 0.0, 10.0, 3.0, step=0.5)
        # Revenue Growth filter requires support in backend filter_stocks
        
    with col3:
        st.subheader("3. 時機 (Timing)")
        low_base_only = st.checkbox("僅顯示「低基期」股票", value=True, help="篩選股價處於近 52 週低點區間（底部 30%）的股票")
    ...
    st.dataframe(
        preview_df[['stock_code', 'stock_name', 'current_price', 'market_cap', 'pe_ratio', 'price_position']].style.format({
            'current_price': '{:.2f}',
            'market_cap': '{:,.0f}',
            'pe_ratio': '{:.2f}',
            'price_position': '{:.2f}'
    ...
    with col_d1:
        min_rev_cagr = st.slider("最低營收成長率 (5年 CAGR)", -10.0, 30.0, 0.0, step=1.0)
        min_profit_cagr = st.slider("最低獲利成長率 (5年 CAGR)", -10.0, 30.0, 5.0, step=1.0)
    with col_d2:
        min_div_years = st.slider("連續配息最低年數", 0, 20, 5)
