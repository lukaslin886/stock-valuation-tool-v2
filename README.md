...
⚡ **[公式速查表](FORMULA_REFERENCE.md)** - 快速查詢所有計算公式與參數  
📊 **[時間加權成長率說明](TIME_WEIGHTING.md)** - EWMA 時間加權方法完整解析與視覺化 ✨ **NEW**
...
### 第三章：時間加權成長率 ✨ **NEW**
- **EWMA 理論** - 指數加權移動平均在估值領域的應用
- **近期數據更高權重** - 更準確反映當前趨勢與突發事件
- **參數調校策略** - 根據產業特性調整 `recent_weight_ratio`
- **實務範例** - 台積電、復甦股、衰退股的完整計算
- **統計檢定** - 時間加權 vs 等權重的效果比較
- **視覺化分析** - 提供指數衰減權重分配圖表，直觀顯示數據貢獻度
